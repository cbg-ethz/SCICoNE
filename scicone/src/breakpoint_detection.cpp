...
int main( int argc, char* argv[]) {
    ...
    while ((n_breakpoints <= breakpoints_min_limit) && (try_num <= 2))
    {
      ...
      int smaller_window_size = 1;
      float approx_smaller_window_size = window_size * 0.4;
      if (approx_smaller_window_size >= 1)
        smaller_window_size = static_cast<int>(approx_smaller_window_size);
      ...
      while(!q_map.empty())
      {
          ...
          if (elem.second - elem.first > smaller_window_size)
          {
              ...
              if (max_idx != -1) // -1 means rejected
              {
                  ...
                  {
                      int left = elem.first;
                      int right = max_idx - smaller_window_size;
                      if (window_size == 1)
                        right = max_idx;

                      // compute the left median
                      vector<double> left_vec(sp_cropped.begin() + left, sp_cropped.begin() + right);
                      ...
                      try
                      {
                          int max_left_idx = dsp.find_highest_peak(sp_cropped, left, right);
                          q_map.emplace(sp_cropped[max_left_idx] , std::make_pair(left,right));
                      }
                  ...
                  {
                      int left = max_idx + smaller_window_size + 1;
                      int right = elem.second;
                      if (window_size == 1)
                        left = max_idx + smaller_window_size;

                      // the right median
                      vector<double> right_vec(sp_cropped.begin() + left, sp_cropped.begin() + right);
                      double median_right = MathOp::median(right_vec);
                      // normalise bins on the right by right median
                      for (int j = max_idx + smaller_window_size; j < right; ++j) {
                          sp_cropped[j] /= median_right;
                      }

                      try
                      {
                          int max_right_idx = dsp.find_highest_peak(sp_cropped, left, right);
                          q_map.emplace(sp_cropped[max_right_idx] , std::make_pair(left,right));
                      }
                      catch (const std::runtime_error& e)
                      {
                          std::cerr << e.what() << std::endl;
