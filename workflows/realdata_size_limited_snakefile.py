...
n_bins = 134834
n_cells = 462
n_iters = [100000, 200000, 500000, 1000000, 2000000, 5000000]  # config["cnv_trees"]["n_iters"] # 10000000
ploidy = [2]
n_repetitions = 10

SIM_OUTPUT = "/cluster/work/bewi/members/tuncel/data/dna/recomb_data/recomb_real_data_size_lim"
...
rule all:
    input:
        read_region_sims = expand(SIM_OUTPUT + '_' + 'ploidy_{plo}' +'/'+'{iters}' + 'n_iters_'  + str(n_nodes) + 'nodes_'  + '{regions}'+'regions_'+ '{reads}'+'reads'+ '/'+ \
                '{rep_id}' +'_' + '{output_ext}', plo=ploidy, iters=n_iters,  output_ext=output_file_exts, regions=n_regions,reads=n_reads, rep_id=[str(x)+"_limited" for x in range(0,n_repetitions)])
    output:     
    shell:
...
rule infer_trees:
    ...
    shell:                                                                                                                                                                                                                                                                                                                                                                        "{params.binary} --n_reads {wildcards.reads} --n_nodes {params.n_nodes} --n_regions {params.n_regions} --n_bins {params.n_bins} --n_iters {wildcards.iters} --n_cells \
        {params.n_cells} --verbosity 2 --ploidy {wildcards.plo} --size_limit {params.size_limit} --postfix {wildcards.rep_id} --d_matrix_file {input.d_mat} --region_sizes_file {input.region_sizes}; \
