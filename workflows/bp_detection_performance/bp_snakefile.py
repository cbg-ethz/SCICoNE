...
rule all:
    input:
        ...
        ground_truth = expand(SIM_OUTPUT + str(n_nodes) + 'nodes_' + str(n_regions) + 'regions_' + str(n_reads) + 'reads_' + '{dataset_id}' + '_ground_truth.txt', dataset_id=sim_data)
    output:
    shell:
...
rule simulate:
    params:
        ...
        ground_truth_name = str(n_nodes) + 'nodes_' + str(n_regions) + 'regions_' + str(n_reads) + 'reads_' + '{dataset_id}_ground_truth.txt'
    input:
    output:
        dmatrix = SIM_OUTPUT + str(n_nodes) + 'nodes_' + str(n_regions) + 'regions_' + str(n_reads) + 'reads_' + '{dataset_id}_d_mat.txt',
        ground_truth = SIM_OUTPUT + str(n_nodes) + 'nodes_' + str(n_regions) + 'regions_' + str(n_reads) + 'reads_' + '{dataset_id}_ground_truth.txt'
    ...
    shell:
        ...
        mv {params.ground_truth_name} {output.ground_truth}"
