...
def merge_data_by_chromosome(h5f, key="normalized_counts", downsampling_factor=1):
    ...
    for ch in sorted_chromosome_list:
        chr_matrix = []
        mat = h5f['normalized_counts'][ch][:][0:n_cells]
        n_bins = mat.shape[1]
...
def extract_corrected_counts_matrix(h5f, bins_to_exclude=None, downsampling_factor=1, filter=True):
    downsampling_factor = np.max([1, downsampling_factor])
    unfiltered_counts = merge_data_by_chromosome(h5f, key='normalized_counts', downsampling_factor=downsampling_factor)
